-- TOM-98 Lot A — statut `draft` + garde-fous (backward-compatible, additif, atomique).
--
-- PREUVE DU BESOIN
-- Enum listing_status = active | hidden | suspended | sold.
-- Brouillons étaient en mémoire session uniquement. Réutiliser `hidden` collisionne
-- avec la pause vendeur.
--
-- STRATÉGIE
-- 1) ADD VALUE `draft` (défaut inchangé = active). Discovery client filtre déjà
--    status = 'active' : draft exclu automatiquement.
-- 2) Sale cycle : INSERT draft → pas de cycle ; draft→active → démarre le cycle.
--    Renewal inchangé (draft→active = autre UPDATE, pas de boost renewed_at).
...
-- 4) RLS : AUCUNE policy draft ajoutée. Prod a déjà owner CRUD (user_id = auth.uid())
--    et SELECT public = status active. Ne pas ENABLE/DISABLE RLS. Ne pas élargir
--    le SELECT public.
--
-- VALIDATION REQUISE AVANT APPLICATION EN PRODUCTION
-- Ne pas appliquer sans GO. Vérifier enum + policies SELECT listings existantes.
...
-- ---------------------------------------------------------------------------
-- 2) Sale cycle : draft sans cycle ; draft→active démarre le cycle
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) Saved Search — exception TOM-98 : 1re publication draft→active uniquement
